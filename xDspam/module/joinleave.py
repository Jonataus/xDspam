...
async def join(xspam: Client, e: Message):
    ...
    if chat.isnumeric():
        return await e.reply_text(
            "Can't join a chat with chat id. Give username or invite link."
        )
    try:
...
@Client.on_message(
    filters.user(SUDO_USERS) & filters.command(["react"], prefixes=HNDLR)
)
...
async def react_here(xspam: Client, message: Message):
    ...
    try:
        ...
        return await message.reply_text(
            "» ɪɴᴠᴀʟɪᴅ ʀᴇᴀᴄᴛɪᴏɴ sᴘᴇᴄɪғɪᴇᴅ, ʀᴇᴀᴄᴛᴇᴅ ᴛᴏ ᴛʜᴇ ᴍᴇssᴀɢᴇ ᴡɪᴛʜ : ❤️"
        )
